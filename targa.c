...
/*Esta función escribe la información del encabezado en el archivo. La función tiene algunas manipulaciones 
inusuales porque los enteros en un archivo targa tienen una longitud de dos bytes y los caracteres 
solo tienen una longitud de un byte. Debido a que los archivos Targa deben ser leídos por máquinas de diferentes arquitecturas, se debe 
hacer aritmética para escribir primero los bytes de menor orden usando MOD (%), seguidos de los bytes de mayor orden 
usando DIV (/) para cualquier número entero.*/
